...
public static class Markowitz
{
        ...
        double[,] cov, int maxIter = 1000, double tol = 1e-10)
    {
        ...
        for (int iter = 0; iter < maxIter; iter++)
        {
            ...
            // Maillard et al. (2010) multiplicative update: w_i_new = w_i · sqrt( (vp/n) / (w_i·(Σw)_i) ),
            // whose fixed point satisfies w_i·(Σw)_i = vp/n for all i (equal risk contribution).
            var wNew = new double[n];
            for (int i = 0; i < n; i++)
            {
                double rcRaw = w[i] * sigW[i]; // w_i · (Σw)_i
                wNew[i] = rcRaw > 1e-14 ? w[i] * Math.Sqrt((vp / n) / rcRaw) : w[i];
            }
